...
import { getAllCycles, getAllTasks, updateTask, logTaskHistory } from './utils/storageHelpers'
...
function App() {
  ...
  const handleCycleLogged = async () => {
    ...
    try {
      ...
      } else if (result.mode === 'suggestions' && result.suggestions.length > 0) {
  ...
  const handleTaskMoved = async (task, newDateStr) => {
    try {
      ...
      await updateTask(task.id, { scheduledDate: newDateStr, autoScheduled: false })
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <div className="flex-1 bg-white flex flex-col">
        ...
        <OverloadBanner warnings={visibleWarnings} onDismiss={handleDismissWarning} />
